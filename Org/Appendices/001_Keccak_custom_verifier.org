...
The following code snippet implements the custom Ed25519 verifier, addressing the cryptographic incompatibility identified in [[#impl_tee_enclave_str][Section 4.9.1]], that Keystone attestation report verification relies on, by substituting SHA3-512 for the standard SHA512 hash primitive, thus matching Keystone's Security Monitor signing convention.
